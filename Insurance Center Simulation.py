...
In the event of an accident, two cars are involved, and they may arrive at the center simultaneously or separately.
However, they must be paired in the designated outside queue or waiting parking area before they can start receiving
service.

Initially, a photograph is taken of the cars as evidence. Subsequently, all the relevant information is compiled into
a document. In the next stage, a professional examines the file and provides a verdict. If necessary, there is an option
to submit a complaint, which is then reviewed. Finally, once all the necessary procedures are completed and the case
is closed, the cars can exit the center.

Nominal Closure time of center is set to 18:00, whereas true closure time is delayed as long as center becomes empty of
customers.

Following performance measures are collected to further analyze insurance center:
1. Mean Total Time Spent in System
2. Empty Parking and Documentation Queue Probability
3. Maximum and Mean Queue Length and Waiting Time in Outside, Photography, Inspection, and Complaint Queue.
4. Mean Server Utilization in Photography, Documentation, Inspection, and Complaint Unit.
5. Center's True Closure Time
...
inputs['Single-Arrival Probability'] = 0.3
inputs['Sue Probability'] = 0.1
...
inputs['Distribution Parameter'] = {'Photography Service Time': 6.,  # Mean in minutes
                                    ...
                                    'Documentation Service Time': [5., 6., 7.],  # [minimum, most likely, maximum] in
                                    # minutes
                                    'Inspection Service Time': 9.,  # Mean in minutes
                                    'D1 Service Time': [6., 8., 9.],  # [minimum, most likely, maximum] in minutes
...
def arrival(future_event_list, state, clock, data, customer, customer_type):
    ...
    if state['Server Status']['Photography'] < inputs['Number of Servers']['Photography']:
        ...
        if customer_type == 'Type I':
            ...
            # Paired arrivals before 18:00 are accepted
            if clock <= (18 - 8) * 60:
                # Mean outside queue waiting time
...
def simulation(end_day, print_results=False, export_excel=False, alpha=0.05, n_digits=3):
    # To compute total run time
    tic = time.time()

    if print_results:
        print_header()

    # Performance measures for each day are stored in this dictionary
    performance_measures = dict()

    # Mean total time spent in system
    performance_measures['Mean Total Time Spent in System'] = dict()

    # Empty parking and documentation queue probabilities
    performance_measures['Empty Parking Probability'] = dict()
    performance_measures['Empty Documentation Queue Probability'] = dict()

    # Maximum queue length of outside, photography, inspection, and complaint queues
    performance_measures['Maximum Queue Length'] = dict()

    performance_measures['Maximum Queue Length']['Outside'] = dict()
    performance_measures['Maximum Queue Length']['Photography'] = dict()
    performance_measures['Maximum Queue Length']['Inspection'] = dict()
    performance_measures['Maximum Queue Length']['Complaint'] = dict()

    # Mean queue length of outside, photography, inspection, and complaint queues
    performance_measures['Mean Queue Length'] = dict()

    performance_measures['Mean Queue Length']['Outside'] = dict()
    performance_measures['Mean Queue Length']['Photography'] = dict()
    performance_measures['Mean Queue Length']['Inspection'] = dict()
    performance_measures['Mean Queue Length']['Complaint'] = dict()

    # Maximum waiting time in outside, photography, inspection, and complaint queues
    performance_measures['Maximum Waiting Time'] = dict()

    performance_measures['Maximum Waiting Time']['Outside'] = dict()
    performance_measures['Maximum Waiting Time']['Photography'] = dict()
    performance_measures['Maximum Waiting Time']['Inspection'] = dict()
    performance_measures['Maximum Waiting Time']['Complaint'] = dict()

    # Mean waiting time in outside, photography, inspection, and complaint queues
    performance_measures['Mean Waiting Time'] = dict()

    performance_measures['Mean Waiting Time']['Outside'] = dict()
    performance_measures['Mean Waiting Time']['Photography'] = dict()
    performance_measures['Mean Waiting Time']['Inspection'] = dict()
    performance_measures['Mean Waiting Time']['Complaint'] = dict()

    # Mean server utilization for photography, documentation, inspection, and complaint
    performance_measures['Mean Server Utilization'] = dict()

    performance_measures['Mean Server Utilization']['Photography'] = dict()
    performance_measures['Mean Server Utilization']['Documentation'] = dict()
    performance_measures['Mean Server Utilization']['Inspection'] = dict()
    performance_measures['Mean Server Utilization']['Complaint'] = dict()

    # System's true closure time
    performance_measures['True Closure Time'] = dict()

    for day in range(1, end_day + 1):
        if export_excel:
            # a list of lists. Each inner list will be a row in the Excel output.
            table = []
            # Step reset at each new day
            step = 1
        # Initialization
        state, future_event_list, data = starting_state()
        # To store closure time for each day
        closure_time = 0
        # next round of time advance possible only if there is at least one event in fel
        while len(future_event_list) != 0:
            ...
            # Mean server utilization for each unit;
            # Compute and accumulate server spare time for each unit from previous clock till now
            for server in list(data['Cumulative Stats']['Server Spare Time'].keys()):
                data['Cumulative Stats']['Server Spare Time'][server] += \
                    (inputs['Number of Servers'][server] - state['Server Status'][server]) * \
            ...
            elif current_event['Event Type'] == '6:00':
                six_o_clock(state, data)
            ...
            if print_results:
                ...
                # to be in touch with program while running
                print('End of simulating day ' + str(day))
        ...
        if export_excel:
            ...
            create_excel(
                table,
                excel_main_header,
                name_of_sheet='Day ' + str(day),
                name_of_workbook='Insurance Center Simulation Output-Day ' + str(day)
            )

        # Compute performance measures and store them in performance_measure dictionary
        # Mean total time spent in system for this day
        performance_measures['Mean Total Time Spent in System']['D' + str(day)] = \
            data['Total Time Spent in System'] / (data['Total Paired Customers Serviced'] * 2)

        # Empty parking and documentation queue probabilities
        performance_measures['Empty Parking Probability']['D' + str(day)] = \
            data['Cumulative Stats']['Empty Duration']['Parking'] / closure_time
        performance_measures['Empty Documentation Queue Probability']['D' + str(day)] = \
            data['Cumulative Stats']['Empty Duration']['Documentation'] / closure_time

        # Maximum queue length of outside, photography, inspection, and complaint queues
        for unit in list(performance_measures['Maximum Queue Length'].keys()):
            # Queue lengths other than outside's counted pairs in queue, so multiply count by two
            if unit != 'Outside':
                performance_measures['Maximum Queue Length'][unit]['D' + str(day)] = \
                    data['Cumulative Stats']['Maximum Queue Length'][unit] * 2
            else:
                performance_measures['Maximum Queue Length'][unit]['D' + str(day)] = \
                    data['Cumulative Stats']['Maximum Queue Length'][unit]

        # Mean queue length of outside, photography, inspection, and complaint queues
        for unit in list(performance_measures['Mean Queue Length'].keys()):
            # Queue lengths other than outside's counted pairs in queue, so multiply count by two
            if unit != 'Outside':
                performance_measures['Mean Queue Length'][unit]['D' + str(day)] = \
                    (data['Cumulative Stats']['Area Under Queue Length Curve'][unit] * 2) / closure_time
            else:
                performance_measures['Mean Queue Length'][unit]['D' + str(day)] = \
                    data['Cumulative Stats']['Area Under Queue Length Curve'][unit] / closure_time

        # Maximum waiting time in outside, photography, inspection, and complaint queues
        for queue in list(performance_measures['Maximum Waiting Time'].keys()):
            performance_measures['Maximum Waiting Time'][queue]['D' + str(day)] = \
                data['Cumulative Stats']['Maximum Queue Time'][queue]

        # Mean waiting time in outside, photography, inspection, and complaint queues
        for queue in list(performance_measures['Mean Waiting Time'].keys()):
            performance_measures['Mean Waiting Time'][queue]['D' + str(day)] = \
                data['Cumulative Stats']['Queue Waiting Time'][queue] / data['Cumulative Stats'][
                    'Service Starters'][queue]

        # Mean server utilization for photography, documentation, inspection, and complaint
        for unit in list(performance_measures['Mean Server Utilization'].keys()):
            performance_measures['Mean Server Utilization'][unit]['D' + str(day)] = \
                1 - (data['Cumulative Stats']['Server Spare Time'][unit] /
                     (inputs['Number of Servers'][unit] * closure_time))

        # System's true closure time
        performance_measures['True Closure Time']['D' + str(day)] = closure_time

    # Point estimates and C.I for performance measures
    # Compute t-value
    q = 1 - (alpha / 2)
    df = end_day - 1
    t_value = scipy.stats.t.ppf(q, df)

    # A dictionary to store final mean performance measures
    estimates = dict()
    estimates['Point Estimate'] = dict()
    estimates['Confidence Interval'] = dict()

    # Mean total time spent in system
    point_estimate = np.array(list(performance_measures['Mean Total Time Spent in System'].values())).mean()
    point_estimate = round(point_estimate, n_digits)

    standard_deviation = np.array(list(performance_measures['Mean Total Time Spent in System'].values())).std()
    confidence_interval_half_width = (standard_deviation * t_value) / math.sqrt(end_day)
    confidence_interval_half_width = confidence_interval_half_width

    confidence_interval = (round(point_estimate - confidence_interval_half_width, n_digits),
                           round(point_estimate + confidence_interval_half_width, n_digits))

    estimates['Point Estimate']['Mean Total Time Spent in System'] = point_estimate
    estimates['Confidence Interval']['Mean Total Time Spent in System'] = confidence_interval

    # Empty parking probability
    point_estimate = np.array(list(performance_measures['Empty Parking Probability'].values())).mean()
    point_estimate = round(point_estimate, n_digits)

    standard_deviation = np.array(list(performance_measures['Empty Parking Probability'].values())).std()
    confidence_interval_half_width = (standard_deviation * t_value) / math.sqrt(end_day)
    confidence_interval_half_width = confidence_interval_half_width
    ...
    estimates['Point Estimate']['Empty Parking Probability'] = point_estimate
    estimates['Confidence Interval']['Empty Parking Probability'] = confidence_interval

    # Empty documentation queue probability
    point_estimate = np.array(list(performance_measures['Empty Documentation Queue Probability'].values())).mean()
    point_estimate = round(point_estimate, n_digits)

    standard_deviation = np.array(list(performance_measures['Empty Documentation Queue Probability'].values())).std()
    confidence_interval_half_width = (standard_deviation * t_value) / math.sqrt(end_day)
    confidence_interval_half_width = confidence_interval_half_width
    ...
    estimates['Point Estimate']['Empty Documentation Queue Probability'] = point_estimate
    estimates['Confidence Interval']['Empty Documentation Queue Probability'] = confidence_interval

    # Maximum queue length of outside, photography, inspection, and complaint queues
    for queue in list(performance_measures['Maximum Queue Length'].keys()):
        point_estimate = np.array(list(performance_measures['Maximum Queue Length'][queue].values())).mean()
        point_estimate = round(point_estimate, n_digits)

        standard_deviation = np.array(
            list(performance_measures['Maximum Queue Length'][queue].values())).std()
        confidence_interval_half_width = (standard_deviation * t_value) / math.sqrt(end_day)
        confidence_interval_half_width = confidence_interval_half_width
        ...
        estimates['Point Estimate']['Maximum ' + str(queue) + ' Queue Length'] = point_estimate
        estimates['Confidence Interval']['Maximum ' + str(queue) + ' Queue Length'] = confidence_interval

    # Mean queue length of outside, photography, inspection, and complaint queues
    for queue in list(performance_measures['Mean Queue Length'].keys()):
        point_estimate = np.array(list(performance_measures['Mean Queue Length'][queue].values())).mean()
        point_estimate = round(point_estimate, n_digits)

        standard_deviation = np.array(
            list(performance_measures['Mean Queue Length'][queue].values())).std()
        confidence_interval_half_width = (standard_deviation * t_value) / math.sqrt(end_day)
        confidence_interval_half_width = confidence_interval_half_width
        ...
        estimates['Point Estimate']['Mean ' + str(queue) + ' Queue Length'] = point_estimate
        estimates['Confidence Interval']['Mean ' + str(queue) + ' Queue Length'] = confidence_interval

    # Maximum waiting time in outside, photography, inspection, and complaint queues
    for queue in list(performance_measures['Maximum Waiting Time'].keys()):
        point_estimate = np.array(list(performance_measures['Maximum Waiting Time'][queue].values())).mean()
        point_estimate = round(point_estimate, n_digits)

        standard_deviation = np.array(
            list(performance_measures['Maximum Waiting Time'][queue].values())).std()
        confidence_interval_half_width = (standard_deviation * t_value) / math.sqrt(end_day)
        confidence_interval_half_width = confidence_interval_half_width
        ...
        estimates['Point Estimate']['Maximum Waiting Time in ' + str(queue) + ' Queue'] = point_estimate
        estimates['Confidence Interval']['Maximum Waiting Time in ' + str(queue) + ' Queue'] = confidence_interval

    # Mean waiting time in outside, photography, inspection, and complaint queues
    for queue in list(performance_measures['Mean Waiting Time'].keys()):
        point_estimate = np.array(list(performance_measures['Mean Waiting Time'][queue].values())).mean()
        point_estimate = round(point_estimate, n_digits)

        standard_deviation = np.array(
            list(performance_measures['Mean Waiting Time'][queue].values())).std()
        confidence_interval_half_width = (standard_deviation * t_value) / math.sqrt(end_day)
        confidence_interval_half_width = confidence_interval_half_width
        ...
        estimates['Point Estimate']['Mean Waiting Time in ' + str(queue) + ' Queue'] = point_estimate
        estimates['Confidence Interval']['Mean Waiting Time in ' + str(queue) + ' Queue'] = confidence_interval

    # Mean server utilization for photography, documentation, inspection, and complaint
    for queue in list(performance_measures['Mean Server Utilization'].keys()):
        point_estimate = np.array(list(performance_measures['Mean Server Utilization'][queue].values())).mean()
        point_estimate = round(point_estimate, n_digits)

        standard_deviation = np.array(
            list(performance_measures['Mean Server Utilization'][queue].values())).std()
        confidence_interval_half_width = (standard_deviation * t_value) / math.sqrt(end_day)
        confidence_interval_half_width = confidence_interval_half_width
        ...
        estimates['Point Estimate']['Mean Server Utilization in ' + str(queue)] = point_estimate
        estimates['Confidence Interval']['Mean Server Utilization in ' + str(queue)] = confidence_interval

    # System's true closure time
    point_estimate = np.array(list(performance_measures['True Closure Time'].values())).mean()
    point_estimate = round(point_estimate, n_digits)

    standard_deviation = np.array(list(performance_measures['True Closure Time'].values())).std()
    confidence_interval_half_width = (standard_deviation * t_value) / math.sqrt(end_day)
    confidence_interval_half_width = confidence_interval_half_width
    ...
    estimates['Point Estimate']['System True Closure Time'] = point_estimate
    estimates['Confidence Interval']['System True Closure Time'] = confidence_interval

    # A dataframe to print point estimate and confidence interval of each performance measure
    performance_measures_dataframe = pd.DataFrame(
        index=estimates['Point Estimate'].keys(),
        columns=estimates.keys()
    )
    # Fill dataframe
    performance_measures_dataframe['Point Estimate'] = estimates['Point Estimate'].values()
    performance_measures_dataframe['Confidence Interval'] = estimates['Confidence Interval'].values()
    # End of simulation run time
    toc = time.time()
    # Print results
    print(performance_measures_dataframe)
    print('\n')
    print('Run Time Duration: ' + str(round(toc - tic, n_digits)) + ' seconds.')


simulation(100, print_results=True)
